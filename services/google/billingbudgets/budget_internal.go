...
func diffBudget(c *Client, desired, actual *Budget, opts ...dcl.ApplyOption) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.AllUpdatesRule, actual.AllUpdatesRule, dcl.Info{ObjectFunction: compareBudgetAllUpdatesRuleNewStyle, EmptyObject: EmptyBudgetAllUpdatesRule, OperationSelector: dcl.RequiresRecreate()}, fn.AddNest("NotificationsRule")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
