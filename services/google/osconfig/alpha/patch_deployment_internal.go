...
func (r *PatchDeploymentPatchConfig) validate() error {
	...
	if !dcl.IsEmptyValueIndirect(r.RetryStrategy) {
		if err := r.RetryStrategy.validate(); err != nil {
			return err
		}
	}
	return nil
...
func (r *PatchDeploymentPatchConfigRetryStrategy) validate() error {
	return nil
}
...
func canonicalizePatchDeploymentPatchConfig(des, initial *PatchDeploymentPatchConfig, opts ...dcl.ApplyOption) *PatchDeploymentPatchConfig {
	...
	cDes.RetryStrategy = canonicalizePatchDeploymentPatchConfigRetryStrategy(des.RetryStrategy, initial.RetryStrategy, opts...)
...
func canonicalizeNewPatchDeploymentPatchConfig(c *Client, des, nw *PatchDeploymentPatchConfig) *PatchDeploymentPatchConfig {
	...
	nw.RetryStrategy = canonicalizeNewPatchDeploymentPatchConfigRetryStrategy(c, des.RetryStrategy, nw.RetryStrategy)
...
func canonicalizePatchDeploymentPatchConfigRetryStrategySlice(des, initial []PatchDeploymentPatchConfigRetryStrategy, opts ...dcl.ApplyOption) []PatchDeploymentPatchConfigRetryStrategy {
	...
	if len(des) != len(initial) {
		...
		items := make([]PatchDeploymentPatchConfigRetryStrategy, 0, len(des))
		for _, d := range des {
			cd := canonicalizePatchDeploymentPatchConfigRetryStrategy(&d, nil, opts...)
			if cd != nil {
				items = append(items, *cd)
			}
		}
	...
	items := make([]PatchDeploymentPatchConfigRetryStrategy, 0, len(des))
	for i, d := range des {
		cd := canonicalizePatchDeploymentPatchConfigRetryStrategy(&d, &initial[i], opts...)
		if cd != nil {
			items = append(items, *cd)
		}
	}
...
func canonicalizeNewPatchDeploymentPatchConfigRetryStrategy(c *Client, des, nw *PatchDeploymentPatchConfigRetryStrategy) *PatchDeploymentPatchConfigRetryStrategy {
	...
	if nw == nil {
		if dcl.IsNotReturnedByServer(des) {
			c.Config.Logger.Info("Found explicitly empty value for PatchDeploymentPatchConfigRetryStrategy while comparing non-nil desired to nil actual.  Returning desired object.")
			return des
		}
		return nil
	}
...
func canonicalizeNewPatchDeploymentPatchConfigRetryStrategySet(c *Client, des, nw []PatchDeploymentPatchConfigRetryStrategy) []PatchDeploymentPatchConfigRetryStrategy {
	if des == nil {
		return nw
	}
	var reorderedNew []PatchDeploymentPatchConfigRetryStrategy
	for _, d := range des {
		matchedNew := -1
		for idx, n := range nw {
			if diffs, _ := comparePatchDeploymentPatchConfigRetryStrategyNewStyle(&d, &n, dcl.FieldName{}); len(diffs) == 0 {
				matchedNew = idx
				break
			}
		}
...
func canonicalizeNewPatchDeploymentPatchConfigRetryStrategySlice(c *Client, des, nw []PatchDeploymentPatchConfigRetryStrategy) []PatchDeploymentPatchConfigRetryStrategy {
	if des == nil {
		return nw
	}
	...
	var items []PatchDeploymentPatchConfigRetryStrategy
	for i, d := range des {
		n := nw[i]
		items = append(items, *canonicalizeNewPatchDeploymentPatchConfigRetryStrategy(c, &d, &n))
	}
...
func comparePatchDeploymentPatchConfigNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.RetryStrategy, actual.RetryStrategy, dcl.Info{ObjectFunction: comparePatchDeploymentPatchConfigRetryStrategyNewStyle, EmptyObject: EmptyPatchDeploymentPatchConfigRetryStrategy, OperationSelector: dcl.TriggersOperation("updatePatchDeploymentUpdatePatchDeploymentOperation")}, fn.AddNest("RetryStrategy")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func comparePatchDeploymentPatchConfigRetryStrategyNewStyle(d, a interface{}, fn dcl.FieldName) ([]*dcl.FieldDiff, error) {
	var diffs []*dcl.FieldDiff

	desired, ok := d.(*PatchDeploymentPatchConfigRetryStrategy)
	if !ok {
		desiredNotPointer, ok := d.(PatchDeploymentPatchConfigRetryStrategy)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a PatchDeploymentPatchConfigRetryStrategy or *PatchDeploymentPatchConfigRetryStrategy", d)
		}
		desired = &desiredNotPointer
	}
	actual, ok := a.(*PatchDeploymentPatchConfigRetryStrategy)
	if !ok {
		actualNotPointer, ok := a.(PatchDeploymentPatchConfigRetryStrategy)
		if !ok {
			return nil, fmt.Errorf("obj %v is not a PatchDeploymentPatchConfigRetryStrategy", a)
		}
		actual = &actualNotPointer
	}

	if ds, err := dcl.Diff(desired.Enabled, actual.Enabled, dcl.Info{OperationSelector: dcl.TriggersOperation("updatePatchDeploymentUpdatePatchDeploymentOperation")}, fn.AddNest("Enabled")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		diffs = append(diffs, ds...)
...
func flattenPatchDeploymentPatchConfig(c *Client, i interface{}, res *PatchDeployment) *PatchDeploymentPatchConfig {
	...
	r.RetryStrategy = flattenPatchDeploymentPatchConfigRetryStrategy(c, m["retryStrategy"], res)
...
// flattenPatchDeploymentPatchConfigRetryStrategyMap flattens the contents of PatchDeploymentPatchConfigRetryStrategy from a JSON
// response object.
func flattenPatchDeploymentPatchConfigRetryStrategyMap(c *Client, i interface{}, res *PatchDeployment) map[string]PatchDeploymentPatchConfigRetryStrategy {
	a, ok := i.(map[string]interface{})
	if !ok {
		return map[string]PatchDeploymentPatchConfigRetryStrategy{}
	}
	...
	items := make(map[string]PatchDeploymentPatchConfigRetryStrategy)
	for k, item := range a {
		items[k] = *flattenPatchDeploymentPatchConfigRetryStrategy(c, item.(map[string]interface{}), res)
	}

	return items
}

// flattenPatchDeploymentPatchConfigRetryStrategySlice flattens the contents of PatchDeploymentPatchConfigRetryStrategy from a JSON
// response object.
func flattenPatchDeploymentPatchConfigRetryStrategySlice(c *Client, i interface{}, res *PatchDeployment) []PatchDeploymentPatchConfigRetryStrategy {
	a, ok := i.([]interface{})
	if !ok {
	...
	items := make([]PatchDeploymentPatchConfigRetryStrategy, 0, len(a))
	for _, item := range a {
		items = append(items, *flattenPatchDeploymentPatchConfigRetryStrategy(c, item.(map[string]interface{}), res))
	}
...
// flattenPatchDeploymentPatchConfigRetryStrategy flattens an instance of PatchDeploymentPatchConfigRetryStrategy from a JSON
// response object.
func flattenPatchDeploymentPatchConfigRetryStrategy(c *Client, i interface{}, res *PatchDeployment) *PatchDeploymentPatchConfigRetryStrategy {
	m, ok := i.(map[string]interface{})
	if !ok {
		return nil
	}

	r := &PatchDeploymentPatchConfigRetryStrategy{}

	if dcl.IsEmptyValueIndirect(i) {
		return EmptyPatchDeploymentPatchConfigRetryStrategy
	}
...
func extractPatchDeploymentPatchConfigFields(r *PatchDeployment, o *PatchDeploymentPatchConfig) error {
	...
	vRetryStrategy := o.RetryStrategy
	if vRetryStrategy == nil {
		// note: explicitly not the empty object.
		vRetryStrategy = &PatchDeploymentPatchConfigRetryStrategy{}
	}
	if err := extractPatchDeploymentPatchConfigRetryStrategyFields(r, vRetryStrategy); err != nil {
		return err
	}
	if !dcl.IsNotReturnedByServer(vRetryStrategy) {
		o.RetryStrategy = vRetryStrategy
	}
	return nil
}
...
func extractPatchDeploymentPatchConfigRetryStrategyFields(r *PatchDeployment, o *PatchDeploymentPatchConfigRetryStrategy) error {
	return nil
}
...
func postReadExtractPatchDeploymentPatchConfigRetryStrategyFields(r *PatchDeployment, o *PatchDeploymentPatchConfigRetryStrategy) error {
	return nil
}
