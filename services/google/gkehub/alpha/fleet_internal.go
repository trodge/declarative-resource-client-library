...
func diffFleet(c *Client, desired, actual *Fleet, opts ...dcl.ApplyOption) ([]*dcl.FieldDiff, error) {
	...
	if ds, err := dcl.Diff(desired.DisplayName, actual.DisplayName, dcl.Info{OperationSelector: dcl.TriggersOperation("updateFleetUpdateFleetOperation")}, fn.AddNest("DisplayName")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
	...
	if ds, err := dcl.Diff(desired.ManagedNamespaces, actual.ManagedNamespaces, dcl.Info{OperationSelector: dcl.TriggersOperation("updateFleetUpdateFleetOperation")}, fn.AddNest("ManagedNamespaces")); len(ds) != 0 || err != nil {
		if err != nil {
			return nil, err
		}
		newDiffs = append(newDiffs, ds...)
